...
void main() {
  ...
  baseColor.a = 1.0;
  vec3 rawColor = texture2D(u_tex, remappedUV).rgb;
  float luminance = dot(rawColor, vec3(0.299, 0.587, 0.114));
  float leadMask = smoothstep(0.58, 0.92, luminance);
  ...
  vec3 trailColor = baseColor.rgb * terminalTint * (1.4 + 1.2 * u_intensity);
  vec3 leadColor = vec3(1.0, 0.96, 0.86) * (1.6 + 1.4 * u_intensity);
  vec3 color = mix(trailColor, leadColor, leadMask);
